...
public class PlayerControllerTerrain : PositionController
{
    /*  In this Player Movement class, first we code for the movement of player game object with arrow keys within the terrain. Then it pick up collectible cubes 
        ...
        13 to 38 alphanumeric strings of length 9-15.
    */
    ...
    private static new int totalPalindromes;
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.CompareTag("Collectibles"))       //  compare tag
        {
            ...
            */
            ...
            if (checkIsPalindrome.text[0] == '(' && checkIsPalindrome.text[checkIsPalindrome.text.Length-1] == ')')
            {
                revs = "(";
                for (int i = checkIsPalindrome.text.Length - 2; i > 0; i--)    // to exclude first and last prenthesis
                {
            ...
            if (revs == checkIsPalindrome.text)                 // if reverse matches the actual text           
            {
                ...
                if (palindromeCount == PositionController.totalPalindromes)  //   if count approaches to the palindrom length. that means there will beno further destruction
                {
                    ...
                    collectedPalindrome.text = "That's Nice!!!!!!!!! You collected " + palindromeCount + " Palindromes";     //  show how many collectibles picked.
                }
            }
